...
namespace Microsoft.AspNet.WebHooks
{
        ...
        where TRegistration : class, IRegistration, new()
    {
        ...
        public override async Task<ICollection<WebHook>> GetAllWebHooksAsync(string user)
        {
            ...
            {
                string msg = string.Format(CultureInfo.CurrentCulture, StorageResources.SqlStore_OperationFailed, "Get", ex.Message);
        ...
        public override async Task<WebHook> LookupWebHookAsync(string user, string id)
        {
            ...
            {
                string msg = string.Format(CultureInfo.CurrentCulture, StorageResources.SqlStore_OperationFailed, "Lookup", ex.Message);
                _logger.Error(msg, ex);
                throw new InvalidOperationException(msg, ex);
            }
        }
        ...
        public override async Task<StoreResult> InsertWebHookAsync(string user, WebHook webHook)
        {
            ...
            catch (DbUpdateException uex)
            {
                ...
                string msg = string.Format(CultureInfo.CurrentCulture, StorageResources.SqlStore_SqlOperationFailed, "Insert", error);
                _logger.Error(msg, uex);
                return StoreResult.Conflict;
            }
            catch (OptimisticConcurrencyException ocex)
            {
                string msg = string.Format(CultureInfo.CurrentCulture, StorageResources.SqlStore_ConcurrencyError, "Insert", ocex.Message);
                _logger.Error(msg, ocex);
                return StoreResult.Conflict;
            }
            catch (SqlException sqlex)
            {
                string msg = string.Format(CultureInfo.CurrentCulture, StorageResources.SqlStore_SqlOperationFailed, "Insert", sqlex.Message);
                _logger.Error(msg, sqlex);
                return StoreResult.OperationError;
            }
            catch (DbException dbex)
            {
                string msg = string.Format(CultureInfo.CurrentCulture, StorageResources.SqlStore_SqlOperationFailed, "Insert", dbex.Message);
                _logger.Error(msg, dbex);
                return StoreResult.OperationError;
            }
            catch (Exception ex)
            {
                string msg = string.Format(CultureInfo.CurrentCulture, StorageResources.SqlStore_OperationFailed, "Insert", ex.Message);
                _logger.Error(msg, ex);
        ...
        public override async Task<StoreResult> UpdateWebHookAsync(string user, WebHook webHook)
        {
            ...
            {
                string msg = string.Format(CultureInfo.CurrentCulture, StorageResources.SqlStore_ConcurrencyError, "Update", ocex.Message);
                _logger.Error(msg, ocex);
                return StoreResult.Conflict;
            }
            catch (SqlException sqlex)
            {
                string msg = string.Format(CultureInfo.CurrentCulture, StorageResources.SqlStore_SqlOperationFailed, "Update", sqlex.Message);
                _logger.Error(msg, sqlex);
                return StoreResult.OperationError;
            }
            catch (DbException dbex)
            {
                string msg = string.Format(CultureInfo.CurrentCulture, StorageResources.SqlStore_SqlOperationFailed, "Update", dbex.Message);
                _logger.Error(msg, dbex);
                return StoreResult.OperationError;
            }
            catch (Exception ex)
            {
                string msg = string.Format(CultureInfo.CurrentCulture, StorageResources.SqlStore_OperationFailed, "Update", ex.Message);
                _logger.Error(msg, ex);
                return StoreResult.InternalError;
            }
            return StoreResult.Success;
        ...
        public override async Task<StoreResult> DeleteWebHookAsync(string user, string id)
        {
            ...
            {
                string msg = string.Format(CultureInfo.CurrentCulture, StorageResources.SqlStore_ConcurrencyError, "Delete", ocex.Message);
                _logger.Error(msg, ocex);
                return StoreResult.Conflict;
            }
            catch (SqlException sqlex)
            {
                string msg = string.Format(CultureInfo.CurrentCulture, StorageResources.SqlStore_SqlOperationFailed, "Delete", sqlex.Message);
                _logger.Error(msg, sqlex);
                return StoreResult.OperationError;
            }
            catch (DbException dbex)
            {
                string msg = string.Format(CultureInfo.CurrentCulture, StorageResources.SqlStore_SqlOperationFailed, "Delete", dbex.Message);
                _logger.Error(msg, dbex);
                return StoreResult.OperationError;
            }
            catch (Exception ex)
            {
                string msg = string.Format(CultureInfo.CurrentCulture, StorageResources.SqlStore_OperationFailed, "Delete", ex.Message);
                _logger.Error(msg, ex);
                return StoreResult.InternalError;
            }
            return StoreResult.Success;
        ...
        public override async Task DeleteAllWebHooksAsync(string user)
        {
            ...
            {
                string msg = string.Format(CultureInfo.CurrentCulture, StorageResources.SqlStore_OperationFailed, "DeleteAll", ex.Message);
                _logger.Error(msg, ex);
                throw new InvalidOperationException(msg, ex);
            }
        }
